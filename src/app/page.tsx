      ...
      <section id="pricing" className="py-28 px-6 bg-green-950 relative overflow-hidden">
        ...
        <div className="container mx-auto max-w-4xl relative z-10">
          ...
          <div className="flex flex-col items-center gap-0 max-w-lg mx-auto">
            ...
            {/* ── STEP 1: Foundation ── */}
            <motion.div className="w-full rounded-2xl p-8 relative bg-amber-500/10 border border-amber-400/50 hover:-translate-y-1 transition-transform duration-300" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
              <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-amber-400 text-green-950 text-xs font-bold tracking-widest uppercase px-4 py-1 rounded-full whitespace-nowrap">
                {T.pricing.badge}
              </div>
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-xs font-semibold tracking-widest uppercase mb-2 text-amber-300">{T.pricing.plans[0].plan}</p>
                  {"wasPrice" in T.pricing.plans[0] && T.pricing.plans[0].wasPrice && (
                    <p className="text-sm text-green-200/35 line-through mb-0.5 tracking-wide"><span className="text-green-200/25 text-xs mr-0.5">$</span>{T.pricing.plans[0].wasPrice}</p>
                  )}
                  <div className="text-6xl font-semibold text-[#f6f1e7] leading-none mb-1" style={{ fontFamily: "var(--font-cormorant), serif" }}>
                    <sup className="text-2xl align-super font-normal">$</sup>{T.pricing.plans[0].price}
                  </div>
              ...
              <div className="mt-6 pt-6 border-t border-white/10">
                <ul className="space-y-3 mb-6">
                  {T.pricing.plans[0].features.map((f, j) => (
                ...
                {T.pricing.previewBtn && (
                  <button onClick={() => { setPreviewPlan("standard"); track("report_preview", { type: "standard" }); }} className="w-full text-center py-2 mb-3 rounded-lg text-xs font-medium text-amber-300/70 hover:text-amber-300 border border-amber-400/20 hover:border-amber-400/40 bg-amber-400/[0.04] hover:bg-amber-400/[0.08] transition-all">
                    {T.pricing.previewBtn}
                  </button>
                )}
